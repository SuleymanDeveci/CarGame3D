...
public class CarController : MonoBehaviour
{
    ...
    [SerializeField] private Slider _autoGearSlider;            // otomatik vites için kullanilan bu slider 1, 2, 3, 4 olmak uzere sadece bu 4 int degeri alabilecek: 4(P), 3(R), 2(N), 1(D) viteslerini temsil edecek 
    ...
    private float _speed;          // aracin hiz bilgisini tutan degisken
    private float _speedSmooth;    // _speed degiskeni'nin ani degisimlerden etkilenmeyen hali (arac sesi icin kullaniyoruz)
    ...
    private void Start()
    {
        ...
        InstantiateSmoke();
        _autoGearSlider.onValueChanged.AddListener(delegate { OnAutoGearValueChange(); }); // sliderin degeri her degistiginde, yani her vites degistiginde "OnAutoGearValueChange" fonk. tetiklenir
    ...
    private void Update()
    {
        ...
        _speedSmooth = Mathf.Lerp(_speedSmooth, _speed, Time.deltaTime); // Lerp komutu sayesinde _speed degiskeni'ni biraz geriden takip ediyor ve _speed degiskeni'nin ani degisimlerinden etkilenmeyerek 
    ...
    {
        if(_autoGearSlider.value == 1)  // eger vites D konumunda ise true
        {
        ...
        else if (_autoGearSlider.value == 3) // eger vites R konumunda ise true
        {
            if (Input.GetKey(KeyCode.W)) // "W" ye basili tutuldugu süre boyunca true
            {
    ...
    {
        _steeringAngle = _steeringInput * steeringCurve.Evaluate(_speed); // direksiyon acisini hiz degiskenine gore artirip azaltiyor (maksimum 40 derece - minimum 15 derece Inspectorden ayarlandi)
        if(_autoGearSlider.value == 1) // eger vites D konumunda ise
        {
    ...
    public float GetSpeedRatio() // gasinput degiskeni'nin 0.5 ile 1 arasina clamp yapilmis hali ile arac hizi'nin normalize edilmis halinin carpimini dondurur
    {
        ...
        return (_speedSmooth / _maxSpeed) * gas; // normalize edilmis _speed degiskeni ile gas degiskeni carpilarak return edilir.
